...
// *** GYARA: An cire 'path' da 'app.use(express.static...)' ***
// Tunda vercel.json ne ke aikin rarraba static files
...
app.post('/api/chat', async (req, res) => {
  ...
  if (!GEMINI_API_KEY) {
    console.error('Kuskure: Ba a sa GEMINI_API_KEY a Vercel ba!');
...
app.post('/api/image', async (req, res) => {
  ...
  if (!DEEPAI_API_KEY) {
    console.error('Kuskure: Ba a sa DEEPAI_API_KEY a Vercel ba!');
    return res.status(500).json({ error: 'Server error: Missing API Key' });
  }
